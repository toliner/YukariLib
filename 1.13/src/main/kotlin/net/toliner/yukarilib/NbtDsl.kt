...
/**
 * Because the name "string" conflicts with [INBTBase.getString], this property is named to "str"
 */
...
val NBTTagCompound.longArray
    get() = NBTChildLongArrayManager(this)
...
class NBTChildIntManager internal constructor(private val tag: NBTTagCompound) {
    operator fun set(key: String, value: Int) = tag.setInt(key, value)
    operator fun get(key: String): Int? = if (tag.hasKey(key)) tag.getInt(key) else null
}
...
class NBTChildIntArrayManager internal constructor(private val tag: NBTTagCompound) {
    ...
    operator fun set(key: String, value: List<Int>) = tag.setIntArray(key, value)
    operator fun get(key: String): IntArray? = if (tag.hasKey(key)) tag.getIntArray(key) else null
}

class NBTChildLongArrayManager internal constructor(private val tag: NBTTagCompound) {
    operator fun set(key: String, value: LongArray) = tag.setLongArray(key, value)
    operator fun set(key: String, value: List<Long>) = tag.setLongArray(key, value)
    operator fun get(key: String): LongArray? = if (tag.hasKey(key)) tag.getLongArray(key) else null
}

class NBTChildTagManager internal constructor(private val tag: NBTTagCompound) {
    operator fun set(key: String, value: INBTBase) = tag.setTag(key, value)
    operator fun get(key: String): INBTBase? = if (tag.hasKey(key)) tag.getTag(key) else null
}
