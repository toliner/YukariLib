...
class SpecialSelectedBox {
     ...
     * 指定した側面を描画するかどうか
     */
    fun drawSide(side: Int, draw: Boolean): SpecialSelectedBox {
     ...
     * 指定した側面の方向に四角形を伸ばす
     */
    fun setSideLength(side: Int, length: Double): SpecialSelectedBox {
     ...
     * 指定した値の分だけ広げる
     */
    fun addExtraSpace(space: Double): SpecialSelectedBox {
        this.minX -= space
        this.minY -= space
        this.minZ -= space
    ...
    fun offsetForDraw(x: Double, y: Double, z: Double): SpecialSelectedBox {
        this.minX += x
        this.minY += y
        this.minZ += z
        return this
